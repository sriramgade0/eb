...
const MealPlanner = () => {
  ...
  const generateMealPlan = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404 && err.response?.data?.error?.includes('TDEE')) {
        setError('Please calculate your TDEE first using the Buono Bot before generating a meal plan.');
      } else {
                    ...
                    <Card className="mb-4 border-success">
                      <Card.Body className="bg-light">
                        <Row className="text-center">
                          <Col md={3}>
                            <div className="mb-2">
                              <small className="text-muted d-block">Your Base TDEE</small>
                              <strong className="text-success" style={{fontSize: '1.2rem'}}>{summary.baseTDEE}</strong>
                              <small className="text-muted d-block">cal/day</small>
                            </div>
                          </Col>
                          <Col md={3}>
                            <div className="mb-2">
                              <small className="text-muted d-block">Target Calories</small>
                              <strong className="text-success" style={{fontSize: '1.2rem'}}>{summary.targetCalories}</strong>
                              <small className="text-muted d-block">cal/day</small>
                              <small className="text-muted">
                                ({summary.goal === 'weight-loss' ? '-500 deficit' : '+300 surplus'})
                              </small>
                            </div>
                          </Col>
                          <Col md={6}>
                            <small className="text-muted d-block mb-1">Average Daily Totals</small>
                            <div className="d-flex justify-content-center gap-3">
                              <div>
                                <strong className="text-success">{summary.averageDailyTotals.avgCalories}</strong>
                                <small className="text-muted d-block">Calories</small>
                              </div>
                              <div>
                                <strong className="text-success">{summary.averageDailyTotals.avgProtein}g</strong>
                                <small className="text-muted d-block">Protein</small>
                              </div>
                              <div>
                                <strong className="text-success">{summary.averageDailyTotals.avgCarbs}g</strong>
                                <small className="text-muted d-block">Carbs</small>
                              </div>
                              <div>
                                <strong className="text-success">{summary.averageDailyTotals.avgFats}g</strong>
                                <small className="text-muted d-block">Fats</small>
                              </div>
                            </div>
                          </Col>
                        </Row>
                  ...
                  {mealPlan.filter((dayPlan) => dayPlan.day === currentDay).map((dayPlan) => (
                    <div key={dayPlan.day} className="py-3">
                      <div className="mb-4 text-center">
                        <h5 className="mb-2">Day {dayPlan.day} - Click on a meal to view details</h5>
                        {dayPlan.totals && (
                          <div className="d-flex justify-content-center gap-4 mt-3">
                            <div>
                              <span className="text-muted">Total: </span>
                              <strong className="text-success">{dayPlan.totals.totalCalories} cal</strong>
                            </div>
                            <div>
                              <span className="text-muted">Protein: </span>
                              <strong className="text-success">{dayPlan.totals.totalProtein}g</strong>
                            </div>
                            <div>
                              <span className="text-muted">Carbs: </span>
                              <strong className="text-success">{dayPlan.totals.totalCarbs}g</strong>
                            </div>
                            <div>
                              <span className="text-muted">Fats: </span>
                              <strong className="text-success">{dayPlan.totals.totalFats}g</strong>
                            </div>
                          </div>
                        )}
                      </div>
